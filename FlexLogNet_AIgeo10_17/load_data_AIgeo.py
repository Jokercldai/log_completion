...
class MyGNNDataset(gdata.Dataset):
    def __init__(self, data, sam_len, sam_num, fea_litho, norm_type, filt_size, need_maskd, 
        ...
        self.maskc = maskc
        self.loc_li = loc_li
        self.pred_log = args.pred_log
        print("====== 样本制作中为了能够更好的预测处RHOB曲线，所以选择了 RHOB 完整的样本，因而添加了sam_sub = awfea.iloc[stap:endp,:].copy().iloc[:, 1]句及while循环，可以根据需求更改或者删除")
    ...
    def label_mask0(self,df):
        ...
        sam_sub = awfea.iloc[stap:endp,:].copy().iloc[:, 1]   #[0, 2, 3]

        #不选拼接处的数据  
        while(self.maskc[stap]==0) or (sam_sub.isnull().any()):  #.any()
            stap=np.random.randint(0,len(awfea)-self.sam_len)
            endp=stap+self.sam_len  #endp表示end_point

            """在这里多加了判断,只选用 1 标签完整的样本"""
            sam_sub = awfea.iloc[stap:endp,:].copy().iloc[:, 1]   #[0, 2, 3]
    ...
    def mask_d(self,label,mask0):
        ...
        ###这套数据GR一直是完整的，所以至少是有GR预测DTC的
        maskd=np.ones((label.shape[0],label.shape[1]))

        if self.pred_log == 'RHOB':
            maskd[:, 1] = 0   #将RHOB列掩码掉  GR  RHOB  NPHI  DTC
        elif self.pred_log == 'NPHI':
            maskd[:, 2] = 0
        elif self.pred_log == 'DTC':
            maskd[:, 3] = 0
        elif self.pred_log == 'any':
            '''制作随机的maskd（需要恢复的），起始点随机'''
            maskd=np.ones((label.shape[0],label.shape[1]))
            a=maskd.shape[0]  #a样本长度，512
            b=maskd.shape[1]  #b特征曲线数目，4
...
class MyGNNDataset_cat_sam(gdata.Dataset):
    ...
    def __init__(self, data, sam_len, sam_num, fea_litho, norm_type, mask_logs,
        ...
        print("label_mask0函数中必须设置RHOB初始掩码！！！")
    ...
    def random_single_well(self, df):
        ...
        md_stap = md.iloc[0].values[0]
        #print('====== md_stap',md_stap, type(md_stap))
        md_endp = md.iloc[self.wep].values[0]
