...
from load_data_AIgeo import *    #only use MyGNNDataset  #cldai
...
def read_args():
    ...
    parser.add_argument('--sam_num',default=512, type=int, help='the number of sample in every set') #2560
    parser.add_argument('--epochs', default=501, type=int, help='Number of epochs to train.')

    #数据路径
    parser.add_argument('--filePath',default='/home/cldai/sciresearch/logcomplete/data/AIgeo_1017_process/', 
                        help='file path of train and test')  #cldai add
    parser.add_argument('--train_set',default='train_z_norm_by_norway.csv', #'train_4.csv', 
                        help='用于交叉验证的数据集')
    parser.add_argument('--val_set',default='val_z_norm_by_norway.csv',  #'val_4.csv', 
                        help='用于交叉验证的数据集') 
    parser.add_argument('--pred_log',default="RHOB",type=str,help="需要补全的曲线是: RHOB/NPHI/DTC/any/, 用于load_data_AIgeo文件中mask_d函数制作掩码训练样本")
    parser.add_argument('--finetune',default="FFT",type=str,
                        help="全量微调:FFT  部分微调:Part_FT 重新训练:train  用于是重新训练还是不同的微调方式. 控制预训练模型的调用") 
    

    parser.add_argument("--save_model_path", default='/home/cldai/sciresearch/logcomplete/model/AIgeo/', help="模型保存路径")  #'./model/   maskd CLvTvl
    parser.add_argument("--lossfigpath", default='/home/cldai/sciresearch/logcomplete/figure/AIgeo/loss/', help="loss图存放地址") # maskd  CLvTvl
    parser.add_argument('--sam_len',default=512, type=int, help='训练样本尺寸')  #cldai add
    ...
    parser.add_argument('--need_maskd',default=True, help='')
    parser.add_argument("--loss_name", default='/home/cldai/sciresearch/logcomplete/loss/loss_init.npz', help="Loss_name")
...
def main(args):
    ...
    print(f"学习率由原来的0.001(8.5日前)更新为: {args.LR}")
    print('读取数据路径：',args.filePath)
    print('模型保存在：',args.save_model_path)
    print('注意读取的交叉数据的标号！！')
    print('====== 训练数据集：', args.train_set)
    print('====== 验证数据集：', args.val_set)
    
    
    dfTrain=pd.read_csv(args.filePath+args.train_set,engine='python')
    dfTest=pd.read_csv(args.filePath+args.val_set,engine='python')

    print("抽取WELL DEPTH_MD GR RHOB RDEP DTC, 并将RDEP改名为NPHI !!!")
    dfTrain = dfTrain[['WELL','DEPTH_MD','GR', 'RHOB', 'RDEP', 'DTC']].copy()
    dfTrain = dfTrain.rename(columns = {'RDEP':'NPHI'})
    dfTest = dfTest[['WELL','DEPTH_MD','GR', 'RHOB', 'RDEP', 'DTC']].copy()
    dfTest = dfTest.rename(columns = {'RDEP':'NPHI'})
    print('改columns名后的 columns:', dfTrain.columns)
    ...
    print('===== train_wells_name', train_wells_name)
    ...
    train_dataset = MyGNNDataset(dfTrain, args.sam_len, args.sam_num, args.fea_litho, 
                                ...
                                args.need_maskd, train_maskc, train_loc_li, args)
    test_dataset = MyGNNDataset(dfTest, args.sam_len, args.sam_num, args.fea_litho, 
                                args.data_norm_type, args.filt_size,
                                args.need_maskd, test_maskc, test_loc_li, args,)
    ...
    print('device',device)
    #summary(model, input_size=(args.batch_size, 1, 512))
    # stats_list = profileit(model, input_size=(3, 512))  
    # summary = get_stats_summary(stats_list)
    # print(summary)

    print("Total trainable parameters:", count_parameters(model))
    
    train_network(model,device,args.LR,args.epochs,args.save_model_path,train_loader,
                    test_loader,args.loss_name,args.lossfigpath,args.batch_size,args.fea_litho,args)
...
if __name__ == '__main__':
    ...
    ###保存模型大小
    # model = GraphNet(args.graph_channels,args.batch_size,args.fea_litho,args.head,args.drop)
    # torch.save(model.state_dict(), '/home/cldai/sciresearch/logcomplete/model/model_save/aa.pt')
    
    # layer = AggredGNN(128,128,128,128,dropout=0.1,batchsize = 4,down_up = False,poolstride=2)
    # torch.save(layer.state_dict(), '/home/cldai/sciresearch/logcomplete/model/model_save/layer.pt')

    # layer11 = AggredGNN(512,512,32,32,dropout=0.1,batchsize = 4,down_up = False,poolstride=2)
    # torch.save(layer11.state_dict(), '/home/cldai/sciresearch/logcomplete/model/model_save/layer11.pt')
