...
def whole_gardner(label,fea_litho,md_stap,md_endp,figpath,well_name):  
    ...
    #计算相关系数
    gardner_rhob_rmse, gardner_dtc_rmse, rb_corr, dtc_corr = whole_gardner_corr(figpath,well_name,rhob,dtc,dp_rhob,rp_dtc)

    return gardner_rhob_rmse, gardner_dtc_rmse, rb_corr, dtc_corr
...
def whole_gardner_corr(figpath,well_name,rhob,dtc,dp_rhob,rp_dtc):
    ...
    gardner_rhob_rmse = np.sqrt(((rb_dna.iloc[:, 0] - rb_dna.iloc[:, 1]) ** 2).mean())
    gardner_dtc_rmse = np.sqrt(((dtc_dna.iloc[:, 0] - dtc_dna.iloc[:, 1]) ** 2).mean())
    print('gardner_rhob_rmse:', gardner_rhob_rmse)
    print('gardner_dtc_rmse:', gardner_dtc_rmse)
