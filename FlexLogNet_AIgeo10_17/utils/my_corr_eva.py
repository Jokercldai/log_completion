...
def calculate_MSE(pred,label,fea_litho, mask_log, bilstm=False):
    
    od = fea_litho.index(mask_log)

    if bilstm==False:
        p = pred[od,:]
    elif bilstm==True:
        p = pred
        print('计算bilstm预测数据的MSE，预测数据的shape为', p.shape)
    l = label[od,:]
    #print('预测log的shape', p.shape)

    contain_nan = (True in np.isnan(l))
    print('检验标签log中是否有nan', contain_nan)
    
    dl = np.vstack((p, l))
    #print('未删除nan值前的shape', dl.shape)
    ##删除包含nan值的列
    d = dl[:, ~np.isnan(dl).any(axis=0)]
    #print('删除后data.shape', d.shape)
    eva = np.sqrt(((d[0, :] - d[1, :]) ** 2).mean())
    #print('该井的MSE:', eva)
    
    return eva

def save_pred_label(pred,label,fea_litho, mask_log, md_staps, md_endps, bilstm=None, path=None, ):
    od = fea_litho.index(mask_log)
    p = pred[od,:]
    l = label[od,:]
    print('预测的log的shape', p.shape)
    
    depth = np.linspace(md_staps, md_endps, p.shape[0])
    ...
    if bilstm is not None:
        dl = np.column_stack((depth, l,p,bilstm))
        df = pd.DataFrame(dl, columns=['DEPTH_MD', 'label', 'GNN', 'Bilstm'])
    else:
        dl = np.column_stack((depth, l,p))
        df = pd.DataFrame(dl, columns=['DEPTH_MD', 'label', 'GNN'])

    df['DEPTH_MD'] = df['DEPTH_MD'].round(1)
    print("====== 深度数值保留一位小数")
    print("===== df['DEPTH_MD']", df['DEPTH_MD'])
    df.to_csv(path, index=False)


def bilstm_well_corr(pred,label,fea_litho, mask_log):
    ...
    df_lp = pd.DataFrame(lp, columns=['label', 'pred'])
    print('df_lp.shape,(len,2)', lp.shape)

    corr = df_lp.corr()
    log_corr = corr.iloc[0,1]
    #print('###### 相关系数是{}'.format(log_corr))
...
def whole_well_corr(pred,label,fea_litho,figpath,well_name):
    ...
    return rb_dna_c, nphi_dna_c, dtc_dna_c
