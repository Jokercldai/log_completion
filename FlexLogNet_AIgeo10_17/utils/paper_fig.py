...
import sys
sys.path.append('/home/cldai/sciresearch/logcomplete/utils')
from my_corr_eva import *
...
def plt_state_work(label,figpath):  
    ...
    print('rhob_ip.shape', rhob_ip.shape)
    ...
    print('nphi_ip.shape', nphi_ip.shape)
...
def paper_plt_cat_sam(input,pred,label,mask0,maskd,fea_litho,
    ...
    paper_plot_input_pred_label(figwidth,y,lis,lis1,lis_copy,lis_xlim,fea_litho,md_stap,md_endp,figpath,well_name,mask_logs,
                                rp_dtc, dtc, dp_rhob, rhob, plot_Gardner, bilstm_pred)
...
def paper_plot_input_pred_label(figwidth,y,lis,lis1,lis_copy,lis_xlim,fea_litho,md_stap,md_endp,figpath,well_name,mask_logs,
                                rp_dtc, dtc, dp_rhob, rhob,plot_Gardner, bilstm_pred=None):
    unit = ['API','g/cm\u00B3','ohmm','us/ft']
    if plot_Gardner == True and (mask_logs == 'DTC' or mask_logs == 'RHOB'):
        ...
        if bilstm_pred is not None:
            fig,axes = plt.subplots(1,lis[0].shape[0]+1, figsize=(4.5*figwidth,16), sharex=False,sharey=True,dpi=300)  #sharey=True,y轴刻度值共享
        else:
    ...
    for i in range(lis[0].shape[0]): 
        ...
        if i == fea_litho.index(mask_logs):
            ...
            axes[3].set_title(fea_litho[i]+' (Local)',fontsize=25)  #our method
            axes[3].set_xlabel(unit[i],fontsize=20)
            axes[3].tick_params(labelsize=15) #刻度字体大小20
            if bilstm_pred is not None:
                axes[4].plot(bilstm_pred[0, :],y,color='r')
                axes[4].plot(lis[lis_copy.index('label_1')][i,:],y,color='k',linestyle='dashed')
                axes[4].set_title(fea_litho[i]+' (North sea)',fontsize=25) #BiLSTM
                axes[4].set_xlabel(unit[i],fontsize=20)
                axes[4].tick_params(labelsize=15) #刻度字体大小20
    ...
    ##reversed2的保存路径
    print('预测效果图保存的是reversed2的路径！')
    plt.savefig(figpath+str(well_name)+'input_pred_label.jpg',dpi=300,bbox_inches='tight')
...
def paper_input_3_incom_log(input,pred,label,mask0,maskd,fea_litho,
    ...
    ##计算RMSE
    calculate_3_incom_log_RMSE(pred,label_1,fea_litho)


################ 计算绘制整口井的相关系数,绘制散点图 ################ 计算绘制整口井的相关系数,绘制散点图 ################ 
def calculate_3_incom_log_RMSE(pred,label,fea_litho):
    ...
    rb_rmse = np.sqrt(((rb_dna.iloc[0, :] - rb_dna.iloc[1, :]) ** 2).mean())
    nphi_rmse = np.sqrt(((nphi_dna.iloc[0, :] - nphi_dna.iloc[1, :]) ** 2).mean())
    dtc_rmse = np.sqrt(((dtc_dna.iloc[0, :] - dtc_dna.iloc[1, :]) ** 2).mean())
    print('rb_rmse', rb_rmse)
    print('nphi_rmse', nphi_rmse)
    print('dtc_rmse', dtc_rmse)
