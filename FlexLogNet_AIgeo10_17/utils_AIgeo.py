...
writer = SummaryWriter(log_dir="/home/cldai/sciresearch/logcomplete/tensorboard/AIgeo/") #maskd CLvTvl
...
def train_network(model,device,LR,epochs,model_filename,train_loader,test_loader,loss_name,lossfigpath,bsize,fea_litho, args):
    lr_list = []
    ...
    if args.finetune == "FFT" or args.finetune == "Part_FT":  #需要微调
        '''迁移学习算法'''
        load_model_name = '/home/cldai/sciresearch/logcomplete/model/AIgeo/best_1082epo.pth'
        #load_model_name = '/home/cldai/sciresearch/logcomplete/model/AIgeo/10.18_exp4_pretrain_best_epo67.pth'
        print(f"======== 使用迁移学习！！！,调用模型路径{load_model_name}")
        param = torch.load(load_model_name)['net']
        model.load_state_dict(param)

        if args.finetune == "Part_FT":
        ...
        print(f'======= 总参数数目: {total_params}, 可训练参数数目: {trainable_params}')
    ...
    for epoch in range(epoch_total):
        ...
        ## 保存最好的模型
        if loss_all_test < best_val_loss:
            best_val_loss = loss_all_test
            filename = model_filename + 'best.pth'
            save_model(filename,model,optimizer,epoch)
            print(f'目前最好模型epoch是{epoch}')
            best_epo = epoch
